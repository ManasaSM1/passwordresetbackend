			...
			.findOne({ username }, function (err, result) {
				...
				if (result !== null) {
					...
					let mailOptions = {
						...
						text:
							"The given link will be expires in 1 min: " +
							sendUrl +
							randomString,
					};
		...
		dbo
			...
			.findOne({ string: randomString }, function (err, result) {
				...
				if (result !== null) {
					...
					res.redirect(
						"https://password-reset-ui.netlify.app/resetPassword.html"
					);
				} else {
					console.log({ message: "Link Expired", reset: false });
					res.redirect(
						"https://password-reset-ui.netlify.app/outputPages/invalidLink.html"
					);
				}
				db.close();
			});
